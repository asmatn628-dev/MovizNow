...
export default function UserManagers() {
  ...
  const handleRemoveManager = async (e: React.MouseEvent, userId: string) => {
    e.stopPropagation();
    if (window.confirm('Are you sure you want to remove this user from the User Managers list?')) {
      try {
        await updateDoc(doc(db, 'users', userId), { isUserManager: false });
      } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredManagers.map(manager => (
            <div 
              ...
              className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 cursor-pointer hover:border-emerald-500/50 transition-colors group"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  {manager.photoURL ? (
                    ...
                    <div className="w-12 h-12 bg-zinc-800 rounded-full flex items-center justify-center text-xl font-bold text-emerald-500">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-400">{manager.email}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={(e) => handleRemoveManager(e, manager.uid)}
              ...
              <div className="flex items-center justify-between text-sm text-zinc-500 pt-4 border-t border-zinc-800">
                <div className="flex flex-col">
                  ...
                  <span className={`text-[10px] font-bold uppercase mt-1 ${manager.role === 'user_manager' || manager.role === 'manager' ? 'text-emerald-500' : 'text-zinc-500'}`}>
          ...
          {filteredManagers.length === 0 && (
            <div className="col-span-full p-8 text-center text-zinc-500 bg-zinc-900 border border-zinc-800 rounded-2xl">
