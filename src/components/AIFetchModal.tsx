  ...
  const fetchDataWithAI = async () => {
    ...
    try {
      ...
      if (selectedModel === 'gemini-3-flash') {
        ...
        const data = await aiService.fetchMovieData(initialTitle, String(initialYear || ''), 'movie', null, selectedModel);
        if (data) {
          const ytRes = await ytPromise;
          if (ytRes && ytRes.ok) {
            const ytData = await ytRes.json();
            if (ytData.url) {
              (data as any).trailerUrl = ytData.url;
              (data as any).trailerTitle = ytData.title;
            }
          }
          setFetchedData(data);
        } else {
          throw new Error("Failed to fetch data with selected model.");
        }
      }
    } catch (err: any) {
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-zinc-800 bg-zinc-900/50 shrink-0">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                { id: 'gemini-3-flash', name: 'Gemini 3 Flash', icon: Zap, color: 'text-blue-400' },
                { id: 'perplexity-ai', name: 'Perplexity AI', icon: Globe, color: 'text-emerald-400' },
                { id: 'deepseek-r1', name: 'DeepSeek-R1', icon: Brain, color: 'text-purple-400' },
                { id: 'gpt-5.2-thinking', name: 'GPT-5.2 Thinking', icon: Bot, color: 'text-orange-400' },
                { id: 'openai-o4-mini', name: 'OpenAI o4-mini', icon: Database, color: 'text-zinc-400' },
                { id: 'claude-opus-4.6', name: 'Claude Opus 4.6', icon: Sparkles, color: 'text-amber-400' },
                { id: 'mistral-large-3', name: 'Mistral Large 3', icon: Zap, color: 'text-red-400' },
              ].map((model) => (
                <button
                  key={model.id}
                      ...
                      fetchedData[field] && (
                        <label key={field} className="flex items-start gap-3 p-3 rounded-xl border border-zinc-800 bg-zinc-950/50 cursor-pointer hover:border-zinc-700 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-sm text-white truncate">{fetchedData[field]}</div>
                          </div>
                        </label>
                      )
                    ))}
